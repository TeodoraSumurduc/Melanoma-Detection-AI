...
class config:
    ...
    def data_loader(self, dataset, collate_fn_melanoma=None, shuffle=True):
    ...
    def get_full_dataset(self):
        benign_dataset = MelanomaDataset(self.benign_training_folder, 0, transform=self.get_transform())
        malignant_dataset = MelanomaDataset(self.malignant_training_folder, 1, transform=self.get_transform())
        return ConcatDataset([benign_dataset, malignant_dataset])

    def get_test_dataset(self):
        benign_test = MelanomaDataset(self.benign_testing_folder, 0, transform=self.get_transform())
        malignant_test = MelanomaDataset(self.malignant_testing_folder, 1, transform=self.get_transform())
        return ConcatDataset([benign_test, malignant_test])
    ...
    def train_model(self, dataset):
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
            ...
            val_loader = self.data_loader(val_subset, collate_fn_melanoma, shuffle=False)

            model = Net().to(self.device)
            loss_fn = nn.CrossEntropyLoss(weight=torch.tensor([0.25, 4.0]).to(self.device)) #favorizez melanomul
        ...
        test_loader = self.data_loader(test_dataset, collate_fn_melanoma, shuffle=False)
...
def main():
    cfg = config()
    dataset = cfg.get_full_dataset()
    cfg.train_model(dataset)
